...
class CreateTicketModal extends React.Component {
  ...
  componentDidUpdate() {
    if (this.props.groups && this.props.groups.size > 0) {
      const groups = this.props.groups.map(grp => {
        return { text: grp.get('name'), value: grp.get('_id') }
      }).toArray()

      // Only update if different
      if (groups.length > 0 && this.cachedGroups.length !== groups.length) {
  ...
  render() {
    ...
    const mappedGroups = this.props.groups
      ...
      .toArray()
    console.log('CreateTicketModal: Render. mappedGroups length:', mappedGroups.length);
          ...
          <div className='uk-margin-medium-bottom'>
            <Grid>
              ...
              <GridItem width={allowAgentUserTickets ? '2-3' : '1-1'}>
                ...
                <SingleSelect
                  key={this.cachedGroups.length}
                  showTextbox={false}
                  items={this.cachedGroups}
                  defaultValue={head(this.cachedGroups) ? head(this.cachedGroups).value : ''}
