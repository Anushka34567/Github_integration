    ...
    [
      ...
      function (statuses, next) {
        return self
          ...
          .select('_id date updated priority dueDate')
          .populate('priority')
          .lean()
          .exec(next)
      },
        ...
        const t = _.map(tickets, function (i) {
          return _.transform(
            ...
            function (result, value, key) {
              ...
              if (key === 'priority' && value) result.overdueIn = value.overdueIn
              if (key === 'date') result.date = value
              if (key === 'updated') result.updated = value
              if (key === 'dueDate') result.dueDate = value
            },
            {}
          )
        })
      ...
      function (ids, next) {
        return self
          ...
          .find({ _id: { $in: ids } })
          .limit(50)
          .select('_id uid subject updated date dueDate')
          .lean()
          .exec(next)
      }
    ],
