...
function processReportData(tickets) {
  ...
  for (let i = 0; i < tickets.length; i++) {
    ...
    // Status is populated, use the object directly
    if (ticket.status && ticket.status.name) {
      t.push(ticket.status.name)
    } else {
      // Fallback if status is unpopulated (unlikely) or missing
    ...
    if (ticket.tags) {
      for (let k = 0; k < ticket.tags.length; k++) {
        if (ticket.tags[k] && ticket.tags[k].name) {
...
function processResponse(res, input) {
  ...
  csv.stringify(input, { header: true, columns: headers }).pipe(res)
}
