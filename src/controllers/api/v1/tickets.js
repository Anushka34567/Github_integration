      ...
      function (status, user, done) {
        ...
        ticket.save(function (err, t) {
          ...
          t.populate('group owner priority', function (err, tt) {
            ...
            response.ticket = tt
            require('../../../cache/cache').forceRefresh()
            res.json(response)
          })
        })
      }
      ...
      function (callback) {
        ...
        ticketModel.getTicketsWithObject([groupId], obj, function (err, tickets) {
          ...
          parseTicketStats(req.user.role, tickets, function (data) {
            ...
            var ticketStatusSchema = require('../../../models/ticketStatus')
            ticketStatusSchema.find({ isResolved: true }, function (err, statuses) {
              if (err) return callback(err)
              var resolvedIds = _.map(statuses, function (s) {
                return s._id.toString()
              })

              return callback(null, tickets, resolvedIds)
            })
          })
        })
      },
      function (tickets, resolvedIds, callback) {
        ...
        r.closedTickets = _.filter(tickets, function (v) {
          var sId = v.status._id ? v.status._id.toString() : v.status.toString()
          return resolvedIds.indexOf(sId) !== -1
        })
